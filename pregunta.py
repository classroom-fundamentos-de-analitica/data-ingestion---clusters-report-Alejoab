...
def ingest_data():
    ...
    df['principales_palabras_clave'] = [re.sub('\s+', ' ', x) for x in df['principales_palabras_clave']]
    df['principales_palabras_clave'] = [re.sub(',\s*', ', ', x) for x in df['principales_palabras_clave']]
    df['principales_palabras_clave'] = [x.rstrip('.') for x in df['principales_palabras_clave']]
